...
- **Language selection** (priority chain, documented in `src/i18n/index.ts`): 1) explicit switcher choice on the device (`language` + `language-explicit` in localStorage) → 2) signed-in user's `users.preferred_language` (returned by login/register, applied in `AuthContext`) → 3) first visit only: visitor country via public `GET /api/geo` (Vercel/Cloudflare edge header; LT → `lt`, positive signal only) → 4) browser language → 5) `en`. Explicit choices sync to the DB (login body / `PATCH /api/auth/language`) so server-sent emails (`server/email.ts`) use the right language; logout resets the device to auto-detection.
